...
- For Combined funds, use the project implementation consistently and explain the
...
`S^{CA}_{s,t} = z^F_{s,t-1} * c_{s,t-1}`
where `z^F_{s,t-1}` is the past-only finance-augmented VADER sector sentiment z-score, 
and `c_{s,t-1}` is the prior-day stock-level news coverage ratio:
`c_{s,t-1} = n^{news}_{s,t-1} / N_s`

The purpose of the coverage adjustment is to reduce the influence of sector signals based on sparse stock-level news coverage.
...
- required filenames.
- If the report or app is changed, rebuild or manually check the affected outputs before submission.
